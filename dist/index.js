...
exports.CanvasFileClient = void 0;
...
class CanvasFileClient {
    constructor(api_token, base_url = DEFAULT_API_BASE_URL) {
    ...
    async uploadStream(folder_id, data, metadata) {
        ...
        if (length)
            params.append('size', length.toString());
        if (content_type)
            params.append('content_type', content_type);
        const response = await this.client.post(`/folders/${folder_id}/files`, params.toString(), { headers: this.auth_headers });
        ...
        const file_opts = {};
        if (filename)
            file_opts.filename = filename;
        if (length)
            file_opts.knownLength = length;
        if (content_type)
            file_opts.contentType = content_type;
        form.append('file', data, file_opts);
        const upload_response = await this.client.post(response.data.upload_url, form, {
            headers: form.getHeaders(),
            maxRedirects: 0,
            validateStatus: status => status === HTTP_STATUS_CREATED || (300 <= status && status < 400)
        });
    ...
    async uploadFile(folder_id, pathname) {
        const fstats = await (0, promises_1.stat)(pathname);
        return await this.uploadStream(folder_id, (0, fs_1.createReadStream)(pathname), {
            filename: path.basename(pathname.toString()),
            length: fstats.size
        });
    }
}
exports.CanvasFileClient = CanvasFileClient;
...
async function run() {
    try {
        const apiBaseUrl = core.getInput('api_base_url', { required: true });
        const apiToken = core.getInput('api_token', { required: true });
        const folderId = core.getInput('folder_id', { required: true });
        // eslint-disable-next-line prettier/prettier
        const filenames = core.getMultilineInput('filenames', { required: true });
        const exclude = core.getMultilineInput('exclude_filenames');
